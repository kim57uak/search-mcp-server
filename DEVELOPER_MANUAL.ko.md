...
│ ├── server.js # 주 서버 초기화, HTTP 인터페이스 및 MCP 요청 처리
│ ├── tools/ # MCP 도구 정의
...
- 📄 **`src/server.js`**:
  - Express.js를 사용하여 HTTP 서버를 설정합니다.
  ...
  - `/mcp` 엔드포인트를 통해 HTTP POST 요청을 받아, 해당 요청을 MCP 서버의 `processRequest` (또는 유사한) 메소드로 전달하여 도구를 실행합니다.
  - 최상위 오류 처리 및 Graceful shutdown 로직을 포함합니다.
...
- ✅ **출력 (성공 시 `content[0].text` 내부의 JSON 구조 예시):**
  ```json
  {
    "query": "사용자 검색어",
...
서버 포트(`PORT`) 및 실행 환경(`NODE_ENV`)도 환경 변수를 통해 설정 가능합니다. (자세한 내용은 [INSTALL.md](INSTALL.md) 참조)

## 5. 💪 SOLID 원칙 적용

서버는 SOLID 원칙을 준수하는 것을 목표로 합니다:

- 🎯 **단일 책임 원칙 (SRP)**: 각 모듈(`server.js`, `googleSearchTool.js`, `searchService.js`, `serviceConfig.js`, `logger.cjs`)은 명확히 구분된 책임을 가집니다.
    ...
    export const myNewTool = {
      ...
      async handler({ param1 }) {
        ...
        try {
          ...
          logger.error(`Error in myNewToolName: ${error.message}`, {
            error: error.stack,
          });
          throw error;
        }
...
4.  서버를 재시작하면 새 도구를 사용할 수 있습니다.

## 7. 🚀 실행 및 테스트

서버 실행 지침은 [INSTALL.md](INSTALL.md)를 참조하십시오.
...
- **인증/인가 메커니즘 추가:** 필요시 특정 도구 접근에 대한 인증/인가 로직을 추가할 수 있습니다.
